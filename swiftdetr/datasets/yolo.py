...
from swiftdetr.datasets.coco import (
...
def _build_coco_api_from_samples(classes: list[str], dataset: Any) -> Any:
    ...
    coco_dataset = {
        "info": {"description": "Swift-DETR YOLO dataset"},
        "images": images,
...
class ConvertYolo:
    """
    Converts supervision Detections to the target dict format expected by Swift-DETR.
    ...
    def __call__(self, image: Image.Image, target: dict) -> tuple:
        """
        Convert image and YOLO detections to Swift-DETR format.
...
class YoloDetection(VisionDataset):
    ...
    This class provides a VisionDataset interface compatible with Swift-DETR training,
...
def build_roboflow_from_yolo(image_set: str, args: Any, resolution: int) -> YoloDetection:
    ...
    Args:
        ...
        args: Argument namespace. The following attributes are consumed:
            ...
            :data:`~swiftdetr.datasets.aug_config.AUG_CONFIG`.
