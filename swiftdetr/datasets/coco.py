...
from swiftdetr.datasets.transforms import AlbumentationsWrapper, Normalize
...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    so that :class:`~swiftdetr.datasets.coco_eval.CocoEvaluator` can convert
...
def _build_train_resize_config(
    ...
    *,
    ...
    :func:`~swiftdetr.util.tensors.make_collate_fn`, not here.
...
def make_coco_transforms(
    ...
    The ``SwiftDetrDataModule`` then applies augmentation and normalization on the
    ...
    Args:
        ...
        aug_config: Albumentations augmentation config dict passed to
            :class:`~swiftdetr.datasets.transforms.AlbumentationsWrapper`.  Falls back
            to the default :data:`~swiftdetr.datasets.aug_config.AUG_CONFIG` when
            ``None``.
        gpu_postprocess: When ``True``, skip Albumentations augmentation wrappers and
            ``Normalize`` from the CPU pipeline.  The ``SwiftDetrDataModule`` then applies
    ...
    Returns:
        ...
        .. note::
            ...
            these transforms outside of :class:`~swiftdetr.training.module_data.SwiftDetrDataModule`,
            pass the result through :func:`~swiftdetr.util.tensors.nested_tensor_from_tensor_list`
            with ``block_size=patch_size * num_windows``, or use
            :func:`~swiftdetr.util.tensors.make_collate_fn` with that value.
...
def make_coco_transforms_square_div_64(
    ...
    Args:
        ...
        aug_config: Augmentation configuration dictionary compatible with
            :class:`~swiftdetr.datasets.transforms.AlbumentationsWrapper`. If ``None``,
            the default :data:`~swiftdetr.datasets.aug_config.AUG_CONFIG` is used.
        gpu_postprocess: When ``True``, skip Albumentations augmentation wrappers and
            ``Normalize`` from the CPU pipeline.  The ``SwiftDetrDataModule`` then applies
            both augmentation and normalization on the GPU in
            ``on_after_batch_transfer``.  Has no effect on val/test splits.
...
def _resolve_runtime_augmentation_backend(backend: str) -> str:
    ...
    Thin wrapper around :func:`swiftdetr.datasets.kornia_transforms.resolve_augmentation_backend`
    ...
    """
    from swiftdetr.datasets.kornia_transforms import resolve_augmentation_backend
