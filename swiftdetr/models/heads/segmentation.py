...
from swiftdetr.util.tensors import _bilinear_grid_sample
...
class _DepthwiseConvWithoutCuDNN(torch.autograd.Function):
    ...
    See upstream notes on cuDNN ConvTranspose2d backward engine selection.
    """
...
def point_sample(input: torch.Tensor, point_coords: torch.Tensor, **kwargs: Any) -> torch.Tensor:
    """
    A wrapper around :func:`~swiftdetr.util.tensors._bilinear_grid_sample` to support 3D point_coords tensors.
    ...
    Returns:
        A tensor of shape (N, C, P) or (N, C, Hgrid, Wgrid) that contains
            ...
            interpolation from `input` the same way as :func:`~swiftdetr.util.tensors._bilinear_grid_sample`.
    """
    add_dim = False
