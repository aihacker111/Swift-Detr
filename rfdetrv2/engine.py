...
def map_eval_labels_to_coco_category_ids(
    ...
    """Map model class indices ``0 .. K-1`` to COCO ``category_id`` for pycocotools.

    The LW-DETR head uses ``K + 1`` logits for ``K`` foreground classes (see
    ``build_model``: ``num_classes = args.num_classes + 1``). ``PostProcess``
    can therefore emit label index ``K``, which is **not** a dataset class and
    has no entry in ``label_to_cat_id``. Those predictions are removed; only
    ``0 <= label < K`` are mapped.
    """
    if not label_to_cat_id:
        return results_all
    k = len(label_to_cat_id)
    id_tensor = torch.as_tensor(list(label_to_cat_id), dtype=torch.long)
    out: List[dict] = []
    for res in results_all:
        if "labels" not in res:
            out.append(res)
        ...
        if not valid.any():
            nres = dict(res)
            nres["labels"] = labels.new_empty((0,), dtype=torch.long)
            if "scores" in res:
                nres["scores"] = res["scores"][:0]
            if "boxes" in res:
                nres["boxes"] = res["boxes"][:0]
            if "masks" in res:
                nres["masks"] = res["masks"][:0]
            out.append(nres)
            continue
        if not valid.all():
            nres = dict(res)
            nres["labels"] = labels[valid]
            if "scores" in res:
                nres["scores"] = res["scores"][valid]
            if "boxes" in res:
                nres["boxes"] = res["boxes"][valid]
            if "masks" in res:
                nres["masks"] = res["masks"][valid]
            res = nres
            labels = res["labels"]
        id_dev = id_tensor.to(device=labels.device, non_blocking=True)
        mapped = id_dev[labels.long()]
        out.append({**res, "labels": mapped})
    return out
