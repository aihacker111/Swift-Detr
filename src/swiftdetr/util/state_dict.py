...
def strip_checkpoint(checkpoint: str | os.PathLike[str]) -> None:
    ...
    Preserves ``model_name`` (when present) so that ``SwiftDetr.from_checkpoint()``
    can still resolve the model class from the stripped file.  Also preserves
    ``swiftdetr_version`` (when present) for provenance tracking.
    ...
    # `checkpoint_best_total.pth` is produced by local Swift-DETR training and can
    ...
    # Preserve swiftdetr_version when present for provenance tracking.
    if "swiftdetr_version" in state_dict:
        new_state_dict["swiftdetr_version"] = state_dict["swiftdetr_version"]
    ...
    if (
        ...
        if ckpt_segmentation_head:
            raise ValueError(
                ...
                "Load the weights into a segmentation model (e.g. SwiftDetr with segmentation) instead of a detection model."
            )
        else:
            raise ValueError(
                "The current model has a segmentation head, but the checkpoint was trained without one. "
                "Load the weights into a detection model (e.g. SwiftDetrTiny) instead of a segmentation model."
            )
