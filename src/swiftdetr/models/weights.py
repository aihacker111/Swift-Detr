...
LoRA adapter injection, used by both the L1 inference facade (``swiftdetr.detr``)
and the L2 LightningModule (``swiftdetr.training.module_model``).

The weight-loading logic is taken from ``SwiftDetrModule._load_pretrain_weights``
...
from swiftdetr.assets.model_weights import download_pretrain_weights, validate_pretrain_weights
from swiftdetr.config import ModelConfig, TrainConfig
from swiftdetr.util.decorators import deprecated
from swiftdetr.util.logger import get_logger
from swiftdetr.util.state_dict import _ckpt_args_get, validate_checkpoint_compatibility
...
def load_pretrain_weights(
    ...
    *,
    ...
    # "model." (matching the attribute path inside SwiftDetrModule).  Legacy and
...
def apply_lora(nn_model: torch.nn.Module) -> None:
    ...
    Raises:
        ImportError: If ``peft`` is not installed.
            Install via the Swift-DETR extras, for example::

                pip install "swiftdetr[lora]"
                # or
                pip install "swiftdetr[train]"
    """
    try:
        ...
        raise ImportError(
            ...
            "Install it via Swift-DETR extras, e.g.: "
            'pip install "swiftdetr[lora]" or pip install "swiftdetr[train]".'
        ) from exc
