...
"""Best-model checkpointing and early stopping callbacks for Swift-DETR Lightning training."""
...
from swiftdetr.util.state_dict import _make_fit_loop_state, strip_checkpoint
...
class BestModelCallback(ModelCheckpoint):
    ...
    state is stripped via :func:`swiftdetr.util.misc.strip_checkpoint`.
    ...
    def _build_checkpoint_payload(
        ...
        """Build a PTL-compatible Swift-DETR checkpoint payload.
        ...
        Args:
            ...
            model_name: Name of the model class (e.g. ``"SwiftDetrBase"``).
        ...
        # Record the swiftdetr package version for provenance / compatibility hints.
    ...
    def _get_ema_model_state_dict(
        ...
        Args:
            ...
            pl_module: The ``SwiftDetrModule`` being trained.
    ...
    def _resolve_model_name(pl_module: LightningModule) -> str | None:
        ...
        The CLI/PTL path does not call ``SwiftDetr.train()``, so
        ``model_config.model_name`` may be unset. In that case, infer the model
        class from concrete config names like ``SwiftDetrSmallConfig``.

        Note:
            The ``DeprecatedConfig`` ``RuntimeError`` guard is only reachable
            from the CLI/PTL path. ``SwiftDetr.train()`` pre-populates
        ...
        if config_type_name.startswith("SwiftDetr") and config_type_name.endswith("Config"):
...
class SwiftDetrEarlyStopping(EarlyStopping):
    """Early stopping callback monitoring validation mAP for Swift-DETR.
    ...
    _SYNTHETIC_MONITOR: str = "__swiftdetr_effective_map__"
