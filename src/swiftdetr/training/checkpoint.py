...
Provides :func:`convert_legacy_checkpoint` to convert Swift-DETR ``*.pth``
...
:meth:`swiftdetr.training.module_model.SwiftDetrModule.on_load_checkpoint`.
"""
...
def convert_legacy_checkpoint(old_path: str, new_path: str) -> None:
    """Convert a legacy Swift-DETR ``.pth`` checkpoint to PTL ``.ckpt`` format.
    ...
    * ``state_dict`` keys are prefixed with ``"model."`` to match the
      attribute path inside :class:`~swiftdetr.training.module_model.SwiftDetrModule`.
    ...
    * ``legacy_checkpoint_format: True`` is written so
      :meth:`~swiftdetr.training.module_model.SwiftDetrModule.on_load_checkpoint` can
    ...
    * If an ``ema_model`` key is present it is preserved verbatim under
    ...
    if "model" not in old:
        raise ValueError(
            ...
            " Only Swift-DETR legacy .pth files produced by engine.py are supported."
        )
